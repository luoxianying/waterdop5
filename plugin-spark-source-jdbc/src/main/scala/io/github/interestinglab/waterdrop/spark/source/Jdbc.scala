...
class Jdbc extends SparkBatchSource {
  ...
  def jdbcReader(sparkSession: SparkSession, driver: String): DataFrameReader = {
    ...
    // var tableName = config.getString("table")
    // if(tableName.contains("$$startDate")) {
    //   tableName = tableName.replace("$$startDate", "'"+dateFormat.format(cal.getTime())+"'")
    // }
    // if(tableName.contains("$$endDate")){
    //   tableName = tableName.replace("$$endDate", "'"+dateFormat.format(new Date())+"'")
    // }
